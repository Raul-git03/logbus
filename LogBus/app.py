...
def cadastrar():
    ...
    if request.method == 'POST':
        ...
        arquivo_foto      = salvar_arquivo(foto_perfil, "foto_perfil", aluno_id)
        arquivo_rg_frente = salvar_arquivo(rg_frente, "rg_frente", aluno_id)
        arquivo_rg_verso  = salvar_arquivo(rg_verso, "rg_verso", aluno_id)
        arquivo_endereco  = salvar_arquivo(comprovante_endereco, "comprovante_endereco", aluno_id)
        arquivo_matricula = salvar_arquivo(comprovante_matricula, "comprovante_matricula", aluno_id)
        ...
        for tipo, arquivo in documentos:
            cursor.execute("""
                ...
                VALUES (%s, %s, %s)
            """, (aluno_id, tipo, os.path.basename(arquivo)))
...
def admin():
    ...
    cursor.execute("""
        ...
        WHERE rota = 'goiania' AND status != 'Em espera'
    """)
    ...
    cursor.execute("""
        SELECT COUNT(*) FROM aluno
        WHERE rota = 'trindade' AND status != 'Em espera'
    """)
    ...
    cursor.execute("""
        SELECT DISTINCT faculdade FROM aluno
        WHERE rota = 'goiania' AND status = 'Ativo'
        ORDER BY faculdade
    """)
    ...
    cursor.execute("""
        SELECT DISTINCT faculdade FROM aluno
        WHERE rota = 'trindade' AND status = 'Ativo'
        ORDER BY faculdade
    """)
...
def editar_aluno(id):
    ...
    cursor.execute("""
        UPDATE aluno
        SET nome = %s, telefone = %s, cpf = %s, email = %s,
    ...
    atualizar_documento(id, "foto_perfil",           foto_perfil)
    atualizar_documento(id, "rg_frente",             rg_frente)
    atualizar_documento(id, "rg_verso",              rg_verso)
    atualizar_documento(id, "comprovante_endereco",  comprovante_endereco)
    atualizar_documento(id, "comprovante_matricula", comprovante_matricula)
...
def excluir_aluno(id):
    ...
    pasta_aluno = os.path.join(app.config["UPLOAD_FOLDER"], f"aluno_{id}")
...
def visualizar_documento(aluno_id, tipo):
    ...
    cursor.execute("""
        SELECT nome_arquivo FROM documento
...
def dashboard():
    ...
    resultado_ativos = executar_select("""
        SELECT
            ...
            COUNT(DISTINCT CASE WHEN rota = 'goiania'  THEN faculdade END),
    ...
    return render_template('dashboard.html',
        ...
        mensagem_total_ativoT     = row[2],
        mensagem_total_espera     = row[3],
        mensagem_total_esperaG    = row[4],
        mensagem_total_esperaT    = row[5],
        mensagem_total_faculdade  = row[6],
